...
var (
	...
	App     = kingpin.New("TSAK-3", "[ TSAK-3 ] Telemetry Swiss Army Knife")
	...
	VBanner = App.Flag("banner", "Display [ TSAK-3 ] banner .").Default("false").Bool()

	// SNMP-related configuration
	SNMPCommunity = App.Flag("community", "SNMP 2c community string").Default("public").String()
	IsSNMPAgent   = App.Flag("snmpagent", "Start internal SNMP agent.").Default("false").Bool()
	SNMPListen    = App.Flag("snmplisten", "IP Address for internal SNMP agent").Default("127.0.0.1").String()
	SNMPPort      = App.Flag("snmpport", "Port for internal TSAK SNMP agent").Default("6161").Int()
	IsSNMPTrap    = App.Flag("snmptrapd", "Start internal SNMP Trap receiver.").Default("false").Bool()
	...
	Version = App.Command("version", "Display information about [ TSAK-3 ]")
	VTable  = Version.Flag("table", "Display [ TSAK-3 ] inner information .").Default("true").Bool()

	Shell      = App.Command("shell", "Run TSAK-3 in interactive shell")
	Run        = App.Command("run", "Run TSAK-3 in non-interactive mode")
	Loop       = App.Command("loop", "Execute event loop")
